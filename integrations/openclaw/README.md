...
and reused verbatim, not reimplemented.
...
|---|---|
| `message_sending` | **all** outbound text — proactive sends and replies. Rewrite/cancel before it reaches Slack/Telegram/etc. |
| `before_tool_call` | only the side-effecting tools you list in `governedTools` (default none — we don't rate-limit `read`/`grep`/`ls`) |
...
| `sessionKey` | — | Session to route proactive cron ticks into |
